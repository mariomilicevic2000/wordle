...
export default function GameAnalyzer() {
  ...
  const [bitsOfUncertainty, setBitsOfUncertainty] = useState<number[]>([]);
  ...
  function generatePotentialWords() {
    ...
    const bits: number[] = [];
    bits.push(Math.log2(currentWords.length));
    ...
    <div className="flex flex-col justify-center items-center px-4 py-8 space-y-6">
      <div className="text-center flex flex-col">
        ...
        <input
          ...
          onChange={(e) => setIsNerd(e.target.checked)}
        />
        <label
          htmlFor="nerd-mode"
      ...
      { isNerd && <p className="text-md text-gray-600">Which means there are {Math.log2(potentialSolutions.length).toFixed(2)} bits of uncertainty</p>}
                ...
                {wordsHistory[guessIndex] > 0 ? (
                  <>
                      ...
                      } words from selection</p>
                    {
                      isNerd &&
                      <>
                        <p>In other words, there are {bitsOfUncertainty[guessIndex + 1].toFixed(2)} bits of uncertainty left.</p>
                        <p>You gained {(bitsOfUncertainty[guessIndex] - bitsOfUncertainty[guessIndex + 1]).toFixed(2)} bits of information.</p>
                      </>
                    }
